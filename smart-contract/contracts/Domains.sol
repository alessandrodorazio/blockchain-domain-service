...
contract Domains is ERC721URIStorage {
    ...
    // track ids, provided by openzeppelin
    ...
    constructor(string memory _tld) payable ERC721("2ry Name Service", "2NS") {
    ...
    // domain price, based on the length
    ...
    // add names to the mapping
    function register(string calldata name) public payable {
        if (domains[name] != address(0)) revert AlreadyRegistered();
        if (!valid(name)) revert InvalidName(name);
        require(domains[name] == address(0));
        //check name unregistered
        ...
        string memory json = Base64.encode(
            abi.encodePacked(
                '{"name": "',
                _name,
                '", "description": "A domain on the 2ry name service", "image": "data:image/svg+xml;base64,',
    ...
    modifier onlyOwner() {
        require(isOwner());
        _;
        //any function using the modifier should be executed after the require
    }
